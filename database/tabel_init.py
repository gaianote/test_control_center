...
# 名称，信息，创建日期，修改日期，创建人
database.create_table('PROJECT',ID = 'INTEGER PRIMARY KEY AUTOINCREMENT', NAME = 'TEXT',INFO = 'TEXT',CREATE_DATE = 'TEXT',UPDATE_DATE = 'TEXT',USER = 'TEXT',SORTORDER = 'INTEGER',IS_DELETED = 'INTEGER',IS_EXECUTE = 'TEXT',LOOP = 'INTEGER',PROJECTPATH = 'TEXT',ENTRYORDER = 'TEXT',ENTRYPATH = 'TEXT',REPORTPATH = 'TEXT',TEXTPATH = 'TEXT',)
...
database.create_table('REPORT',ID = 'INTEGER PRIMARY KEY AUTOINCREMENT', NAME = 'TEXT',PROJECT_NAME = 'TEXT',DESCRIPTION = 'TEXT',CREATE_DATE = 'TEXT',STATE = 'TEXT')

database.execute("""
CREATE TRIGGER log_time_project AFTER INSERT ON PROJECT
BEGIN
   UPDATE PROJECT SET SORTORDER = new.ID, CREATE_DATE = datetime('now'),IS_DELETED = 0,LOOP = 1 WHERE ID = new.ID;
END;
""")
database.execute("""
CREATE TRIGGER log_time_group AFTER INSERT ON [GROUP]
BEGIN
   UPDATE [GROUP] SET SORTORDER = new.ID, CREATE_DATE = datetime('now'),IS_DELETED = 0,LOOP = 1 WHERE ID = new.ID;
END;
""")
database.execute("""
CREATE TRIGGER log_time_item AFTER INSERT ON ITEM
BEGIN
   UPDATE ITEM SET SORTORDER = new.ID, CREATE_DATE = datetime('now'),IS_DELETED = 0,LOOP = 1 WHERE ID = new.ID;
END;
""")
database.execute("""
CREATE TRIGGER log_time_step AFTER INSERT ON STEP
BEGIN
   UPDATE STEP SET SORTORDER = new.ID, CREATE_DATE = datetime('now'),IS_DELETED = 0,LOOP = 1 WHERE ID = new.ID;
END;
""")